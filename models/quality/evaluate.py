"""Evaluate entrypoint for models/quality — scaffold only.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate quality model (scaffold)")
    parser.add_argument("--config", default=str(Path(__file__).with_name("config.yaml")))
    parser.add_argument(
        "--out",
        default=str(ROOT / "datasets" / "evaluation" / "quality_latest.json"),
    )
