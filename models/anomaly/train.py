"""Train entrypoint for models/anomaly — scaffold only.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train anomaly model (scaffold)")
    parser.add_argument("--config", default=str(Path(__file__).with_name("config.yaml")))
    args = parser.parse_args()
    print(f"[anomaly] train scaffold — config={args.config}")
